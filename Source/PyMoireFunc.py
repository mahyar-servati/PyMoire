...
def real_to_reciprocal(lat):
    ...
    return rec_lat
#--------------------------------------------First Briloen Zone
def first_brillouin(rec_lat):
    BZ = np.zeros((6+1,2))
    K1x = (rec_lat[0][0] + rec_lat[1][0])/3
    K1y = (rec_lat[0][1] + rec_lat[1][1])/3
    for i in range(6+1):
       BZ[i][0] = K1x*np.cos(np.radians(60*i))-K1y*np.sin(np.radians(60*i))
       BZ[i][1] = K1x*np.sin(np.radians(60*i))+K1y*np.cos(np.radians(60*i))
    return BZ

def first_brillouin2(rec_lat):
    BZ = np.zeros((6+1,2))
    K1x = (2*rec_lat[0][0] + rec_lat[1][0])/3
    K1y = (2*rec_lat[0][1] + rec_lat[1][1])/3
    for i in range(6+1):
       BZ[i][0] = K1x*np.cos(np.radians(60*i))-K1y*np.sin(np.radians(60*i))
       BZ[i][1] = K1x*np.sin(np.radians(60*i))+K1y*np.cos(np.radians(60*i))
    return BZ
...
def moire_vec(MA, M_angle):
 ...
 #--------------------------------------------k-path
# @time_decor
def k_path(mesh_dens, Kx1, Ky1):
    K1 = np.sqrt(pow(Kx1, 2) + pow(Ky1, 2))
    Kx2 = Kx1*np.cos(np.radians(60))-Ky1*np.sin(np.radians(60))
    Ky2 = Kx1*np.sin(np.radians(60))+Ky1*np.cos(np.radians(60))
    step = K1/mesh_dens
    slop = np.arctan((Ky2-Ky1)/(Kx2-Kx1))
    Mx1 = (Kx1+Kx2)/2
    My1 = (Ky1+Ky2)/2
    Mkx_dir = []; Mky_dir = []; Mk_dir = []
    ik = K1
    while ik > 0.0:
        Mkx_dir.append(ik*np.cos(np.arctan(Ky1/Kx1)))
        Mky_dir.append(ik*np.sin(np.arctan(Ky1/Kx1)))
        ik -= step
        
    M1 = (np.sqrt(pow(Mx1, 2) + pow(My1, 2)))
    ik = 0.0
    while ik < M1:
        Mkx_dir.append(ik*np.cos(np.arctan(My1/Mx1)))
        Mky_dir.append(ik*np.sin(np.arctan(My1/Mx1)))
        ik += step
    
    dir3_len = K1/2     
    ik = 0.0
    while ik < dir3_len:
        Mkx_dir.append(Mx1-ik*np.cos(slop))
        Mky_dir.append(My1-ik*np.sin(slop))
        ik += step 
    for i in range(len(Mkx_dir)):
        Mk_dir.append(np.sqrt(pow(Mkx_dir[i], 2)+pow(Mky_dir[i], 2)))
    kpath = np.array([np.array(Mkx_dir), np.array(Mky_dir)]).T
    return kpath
...
def intra_distance(Rlat, Ratm):
    RA, RB = ref_latt(Rlat, Ratm, 3, 3)
    daa=[]; sdaa=[]; ssdaa=[]
    dab=[]; sdab=[]; ssdab=[]
    RAr = []
    RBr = []
    for i in range(len(RA)):
         RAr.append(round(np.sqrt(np.dot(RA[i],RA[i])),5))
         RBr.append(round(np.sqrt(np.dot(RB[i],RB[i])),5))
    set_RAr = list(set(RAr))
    sort_RAr = sorted(set_RAr)
    set_RBr = list(set(RBr))
    sort_RBr = sorted(set_RBr)
    for i in np.arange(len(RA)):
        if round(np.sqrt(np.dot(RA[i],RA[i])),5)==sort_RAr[1]:
            daa.append(RA[i])
        if round(np.sqrt(np.dot(RA[i],RA[i])),5)==sort_RAr[2]:
            sdaa.append(RA[i])
        if round(np.sqrt(np.dot(RA[i],RA[i])),5)==sort_RAr[3]:
            ssdaa.append(RA[i])
    for i in np.arange(len(RB)):
        if round(np.sqrt(np.dot(RB[i],RB[i])),5)==sort_RBr[0]:
            dab.append(RB[i])
        if round(np.sqrt(np.dot(RB[i],RB[i])),5)==sort_RBr[1]:
            sdab.append(RB[i])
        if round(np.sqrt(np.dot(RB[i],RB[i])),5)==sort_RBr[2]:
            ssdab.append(RB[i])
    dAA = np.array(daa); sdAA = np.array(sdaa); ssdAA = np.array(ssdaa)
    dAB = np.array(dab); sdAB = np.array(sdab); ssdAB = np.array(ssdab)
    return dAA, sdAA, ssdAA, dAB, sdAB, ssdAB
