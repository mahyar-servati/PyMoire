...
mesh_dens = 20                  # Moire First Brillouin Zone mesh density

# Reference layer Unitcell
Rlat = np.array([[2.1304215583, -1.2299994602],
                 [0.0000000000, 2.4599989204]])

# Reference Atomic Position
Ratm = np.array([[0.0000000000, 0.000000000],
                 [1.4202810389, 0.000000000]])

# Reference lattice constant
A = round(np.sqrt(np.dot(Rlat[0],Rlat[0])), 10)
...
MBZ = pm.first_brillouin2(Mrlat)
Mkpath = pm.k_path(mesh_dens, MBZ[-2][0], MBZ[-2][1])
end_time = time.time()
...
intra_AB = np.array([[ 1.42028104,  0.0       , 0.0],
                     [-0.71014052,  1.22999946, 0.0],
                     [-0.71014052, -1.22999946, 0.0]])

intra_BA = np.array([[-1.42028104,  0.0       , 0.0],
                     [ 0.71014052,  1.22999946, 0.0],
                     [ 0.71014052, -1.22999946, 0.0]])

intra_AA = np.array([[ 2.1304215583, -1.2299994602, 0.0],
                     [ 0.0         ,  2.4599989204, 0.0],
                     [ 2.1304215583,  1.2299994602, 0.0],
                     [-2.1304215583,  1.2299994602, 0.0],
                     [-2.1304215583, -1.2299994602, 0.0],
                     [ 0.0         , -2.4599989204, 0.0]])
intra_BB = intra_AA

intra_AB_T = pm.twist3D(teta, intra_AB)
intra_BA_T = pm.twist3D(teta, intra_BA)
intra_AA_T = pm.twist3D(teta, intra_AA)
intra_BB_T = pm.twist3D(teta, intra_BB)
...
for n,k in enumerate(red_Mkpath3D):
    ...
    for hopping in hoppings:
        ...
        j=hopping[4]
        length=hopping[6]
        phase=np.exp((2.0j)*np.pi*np.dot(kpnt,length))
        amp=amp*phase
        ham[i,j]+=amp
    ham_use=ham
...
Ef = {6.01:1.68, 5.09:1.69, 4.41:1.69, 3.89:1.695,
      3.48:1.7, 3.15:1.705, 2.88:1.707, 1.05:1.717} 
E_Ef = int_evals - Ef[round(angle, 2)]

# Writing Band-structure in file
band_file = open(f"Moire_band_{round(angle, 2)}-degree.dat", "w")
...
#---------------------------- DOS_Tetrahedron calculation
start_time = time.time()
print('Solving DOS Hamiltonian ....')
eng_div = 80 # number of energy divisions (Its important to DOS accuracy)
...
hs = np.array([k[0], k[20], k[38], k[-1]])
hstitle = ["K", "\u0393", "M", "K'"]  #insert high-symmetry kpoints as a list
...
fig5.tight_layout()
plt.show()
fig5.savefig(f"Moire_BandStructure_DOS_{round(angle, 2)}-degree.png", bbox_inches = 'tight', dpi=300)